	...
	if (isset($cid)) {
		if (isset($_POST)){
			...
			if ($result["status"]=="ok") {
				...
				header("Location:../index.php?status=ok&msgResp=". urlencode($result["msg"]));
			} else {
				header("Location:../loginRegistrazioneResp.php?status=ko&msgResp=" . urlencode($result["msg"]));
			}

		}

	} else {
		header("Location:../index.php?status=ko&msgResp=". urlencode("errore di connessione al db"));
	}
